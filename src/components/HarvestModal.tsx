...
const HarvestModal = ({ isOpen, wisdom, sharing, communities, onWisdomChange, onSharingChange, onFinalize }: HarvestModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#2c2c2a]/95 backdrop-blur-xl animate-in fade-in duration-700">
      <div className="bg-[#fdfbf7] w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
        <div className="bg-[#d4af37] p-8 flex items-center justify-center md:w-1/3">
          <Wheat size={64} className="text-[#2c2c2a] animate-bounce"/>
        </div>
        <div className="p-8 md:w-2/3 space-y-6">
          <div>
            <h2 className="font-serif text-3xl italic text-[#2c2c2a]">The Harvest</h2>
